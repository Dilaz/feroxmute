//! Cohere provider implementation using rig-core
...
/// Cohere provider using rig-core
...
impl CohereProvider {
    /// Create a new Cohere provider from COHERE_API_KEY env var
    pub fn new(model: impl Into<String>, metrics: MetricsTracker) -> Result<Self> {
        let api_key = std::env::var("COHERE_API_KEY")
            .map_err(|_| Error::Provider("COHERE_API_KEY not set".to_string()))?;

        let client = cohere::Client::new(&api_key)
            .map_err(|e| Error::Provider(format!("Failed to build Cohere client: {}", e)))?;
    ...
    pub fn with_api_key(
        ...
        let client = cohere::Client::new(api_key.into())
            .map_err(|e| Error::Provider(format!("Failed to build Cohere client: {}", e)))?;
...
impl LlmProvider for CohereProvider {
    ...
    async fn complete(&self, request: CompletionRequest) -> Result<CompletionResponse> {
        ...
        let response = agent
            .prompt(&prompt)
            .await
            .map_err(|e| Error::Provider(format!("Cohere completion failed: {}", e)))?;

        let estimated_input = prompt.len() as u64 / 4;
        let estimated_output = response.len() as u64 / 4;
        let pricing = PricingConfig::load();
        let cost = pricing.calculate_cost("cohere", &self.model, estimated_input, estimated_output);
        self.metrics
...
mod tests {
    ...
    fn test_provider_requires_api_key() {
        ...
        let result = CohereProvider::new("command-r-plus", MetricsTracker::new());
